...
class AdaptiveCBFEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # 1. Unpack the AI's requested action
        ...
        try:
            ...
            safe_u = u.value
            if safe_u is None:
                safe_u = np.array([0.0, 0.0]) # Brake if solver fails
        except Exception:
        ...
        # Pass alpha out in the info dict so we can plot it later
    ...
    def _get_obs(self):
        return np.array([
            ...
            float(self.target_radius) # NEW 8th value
        ], dtype=np.float32)

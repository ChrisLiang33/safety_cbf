...
if __name__ == "__main__":
    ...
    # Create a 3x2 grid of subplots (15 inches tall to fit everything cleanly)
    fig, axs = plt.subplots(3, 2, figsize=(16, 15))
    fig.suptitle(r"Adaptive CBF Robustness: Dynamic $\alpha$ Across Multiple Scenarios", fontsize=18)
    
    for i, scen in enumerate(scenarios):
        print(f"Running {scen['name']}...")
        obs, info = env.reset()
        
        # Override environment with the current scenario parameters
        ...
        if i == 0: # Only put legend on the first row to keep the layout clean
            ax_alpha.legend(loc="lower left")
        ax_alpha.grid(True)

    plt.tight_layout()
    eval_dir = "./eval_plots/"
    os.makedirs(eval_dir, exist_ok=True)
    save_path = os.path.join(eval_dir, "multi_scenario_dashboard.png")

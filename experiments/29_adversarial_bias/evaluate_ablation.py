"""
ISSf-CBF evaluation with ADVERSARIAL BIAS (toward nearest obstacle) + RANDOMIZED OBSTACLES.
No alpha-only baseline -- focus on alpha/phi behavior analysis.

Eval: worst-case radius error = -1.0, adversarial bias magnitude = 0.7.
...
def setup_scenario(env, scen, radius_error):
    """Setup scenario with radius error and adversarial bias magnitude."""
    env.reset()
    ...
    env.bias_magnitude = EVAL_BIAS_MAGNITUDE
    return env._get_obs()
...
if __name__ == "__main__":
    ...
    print(f"Loading model... (radius err={EVAL_RADIUS_ERROR}, adversarial bias mag={EVAL_BIAS_MAGNITUDE})")
    env = PhiCBFAdversarialBiasEnv()
    ...
    fig.suptitle(rf"ADVERSARIAL BIAS (toward nearest obs) + RANDOM OBS (true={TRUE_RADIUS}, "
                 rf"est={EVAL_ESTIMATED_RADIUS}, bias mag={EVAL_BIAS_MAGNITUDE}): ISSf-CBF Analysis",
                 fontsize=18, y=1.005)
    ...
    for i, data in enumerate(all_scenarios):
        ...
        # Adversarial bias text (no fixed arrow -- direction changes each step)
        ax.text(5, 3, f"Adversarial bias={EVAL_BIAS_MAGNITUDE}", fontsize=7, color="gray",
                bbox=dict(boxstyle='round,pad=0.3', facecolor='lightyellow', alpha=0.9))

        ax.text(5, -12, f"TRUE r={TRUE_RADIUS}, EST r={EVAL_ESTIMATED_RADIUS}\n"
                        f"Gap={TRUE_RADIUS - EVAL_ESTIMATED_RADIUS}m + adv bias={EVAL_BIAS_MAGNITUDE}",
    ...
    fig.suptitle(rf"Aggregate -- ADVERSARIAL BIAS (toward nearest obs) + RANDOM OBS: "
    ...
    # =====================================================================
    print(f"\n{'='*80}")
    print(f"HAND-PICKED SCENARIOS (radius err={EVAL_RADIUS_ERROR}, adversarial bias mag={EVAL_BIAS_MAGNITUDE})")
    print(f"{'='*80}")

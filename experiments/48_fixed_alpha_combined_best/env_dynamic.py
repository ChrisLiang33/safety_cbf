"""
Exp 48: Fixed alpha + higher speed + two-level sensor error + collision -250.
Agent controls [kx, ky, phi]. Alpha is FIXED at 1.0.

Based on Exp 46 (combined best) with alpha removed from action space.
Phi is the ONLY safety knob. Combines fixed alpha (Exp 43) with all
the best training conditions discovered so far.
...
class FixedAlphaCombinedBestEnv(gym.Env):

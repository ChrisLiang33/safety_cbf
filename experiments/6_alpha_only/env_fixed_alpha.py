"""
Single-obstacle fixed-alpha CBF environment with fixed proportional controller.
No agent action needed — alpha and k_nom are both fixed.
Used as baseline to compare against alpha-only dynamic agent.
"""
...
class FixedAlphaEnv(gym.Env):
    def __init__(self, alpha=1.0):
        super().__init__()
        self.dt = 0.1
        self.fixed_alpha = alpha
        self.k_nom_speed = 2.0  # same fixed speed as dynamic env

        # ACTION: dummy — agent has no control, but SB3 needs an action space
        # We use a tiny 1D action that gets ignored
    ...
    def step(self, action):
        # action is ignored — both alpha and k_nom are fixed

"""
Exp 74 Phase 1: Learn PHI only (alpha fixed at 2.0).

Curriculum learning: phi learns the 'safety bubble' pattern first.
Alpha is constant at 2.0 so phi MUST adapt to handle obstacle encounters.

Discrete actions: 3 choices (phi_down / phi_stay / phi_up), step=0.5.
...
# Fixed alpha -- NOT learnable in Phase 1
ALPHA_FIXED = 2.0
...
class PhiOnlyEnv(gym.Env):
    def __init__(self, systematic_bias_range=(-0.6, 0.4), jitter_range=(-0.2, 0.2),
        ...
        # Persistent phi state (alpha is fixed)
        self.phi = PHI_INIT
        ...
        # ACTION: Discrete(3) = phi_down / phi_stay / phi_up
        self.action_space = spaces.Discrete(3)

        # OBSERVATION: 3 obs x 3 + target 3 + velocity 2 + phi 1 = 15
        # No alpha in obs since it's fixed
        self.observation_space = spaces.Box(
            low=np.array([-180, -35, 0, -180, -35, 0, -180, -35, 0,
                          -180, -35, 0.1, -7, -7,
                          PHI_MIN], dtype=np.float32),
            high=np.array([180, 35, 10, 180, 35, 10, 180, 35, 10,
                           180, 35, 5.0, 7, 7,
                           PHI_MAX], dtype=np.float32),
            dtype=np.float32,
        )

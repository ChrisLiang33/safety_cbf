"""
Exp 74: Sequential Phi->Alpha -- fully randomized eval.
Loads BOTH models: Phase 1 phi model (frozen) + Phase 2 alpha model.
Uses AlphaWithFrozenPhiEnv which queries the phi model internally.

3-column layout:
  Col 1: trajectory colored by alpha
  Col 2: alpha + phi time series with obstacle proximity zones
  Col 3: two stacked subplots -- alpha actions and phi actions
...
from env_phase2_alpha import AlphaWithFrozenPhiEnv, ALPHA_INIT, PHI_INIT

MODEL_PATH = "./models_dynamic/alpha_frozen_phi_model"
MAX_STEPS = 800
N_OBS = 3
...
def plot_discrete_actions(ax_alpha, ax_phi, scen, r, fontsize_title=12, fontsize_label=11):
    ...
    ax_alpha.set_title("Alpha Actions (learned)", fontsize=fontsize_title)
    ...
    ax_phi.set_title("Phi Actions (frozen Phase 1)", fontsize=fontsize_title)
...
if __name__ == "__main__":
    ...
    print("Loading alpha model (Phase 2)...")
    env = AlphaWithFrozenPhiEnv()
    ...
    fig.suptitle("Exp 74: Sequential Phi->Alpha (degraded A*, step=0.5)",
                 fontsize=18, y=1.005)

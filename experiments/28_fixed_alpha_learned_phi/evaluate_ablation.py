"""
FIXED alpha + LEARNED phi (ISSf-CBF) evaluation.
Alpha is constant (FIXED_ALPHA=1.0); agent only controls [kx, ky, phi].

Eval: radius noise = -1.0, constant bias = 0.7 (upward for hand-picked).

Outputs:
  plots/combined_scenarios.png  (6-col: traj | phi+obs_zone | speed | alpha_line+dist | phi+dist | policy map)
...
def setup_scenario(env, scen, radius_error, bias=None):
    """Setup scenario with a specific radius estimation error and constant bias."""
    env.reset()
    ...
    if bias is not None:
        env.bias = np.array(bias, dtype=np.float64)
    else:
        env.bias = np.zeros(2)
    return env._get_obs()
...
def run_episode(env, model, scen, radius_error, bias=None):
    """Run episode with [kx, ky, phi] action. Alpha is FIXED_ALPHA (constant)."""
    obs = setup_scenario(env, scen, radius_error, bias=bias)
    ...
    for step in range(MAX_STEPS):
        ...
        alpha_val = FIXED_ALPHA  # constant, NOT from action
        phi_val = float(action[2])  # action is [kx, ky, phi]
...
def generate_random_scenarios(n, seed=42):
    """Generate random scenarios with random obstacle placement, radius error, and bias."""
    ...
    for i in range(n):
        ...
        bias_mag = rng.uniform(0.0, EVAL_BIAS_MAGNITUDE)
        bias = np.array([bias_mag * np.cos(bias_angle), bias_mag * np.sin(bias_angle)])
        scenarios.append({
            "name": f"Random_{i}",
            "obs": placed,
...
if __name__ == "__main__":
    ...
    print(f"FIXED_ALPHA={FIXED_ALPHA}, bias={EVAL_BIAS_MAGNITUDE} (upward for hand-picked)")
    env = FixedAlphaLearnedPhiEnv()
    model = PPO.load(PHI_MODEL_PATH)

    # --- Hand-picked scenarios (upward bias) ---
    HAND_PICKED_BIAS = np.array([0.0, EVAL_BIAS_MAGNITUDE])  # upward
    print(f"\nRunning {len(SCENARIOS)} hand-picked scenarios "
          f"(radius error={EVAL_RADIUS_ERROR}, bias={HAND_PICKED_BIAS})...")
    all_scenarios = []
    for scen in SCENARIOS:
        result = run_episode(env, model, scen, EVAL_RADIUS_ERROR, bias=HAND_PICKED_BIAS)
        all_scenarios.append({"scen": scen, "result": result})

    # =====================================================================
    # COMBINED PLOT: 6 columns
    # Traj | Phi+ObsZone | Speed | FixedAlpha+Dist | Phi+Dist | Policy Map
    # =====================================================================
    ...
    fig.suptitle(rf"FIXED $\alpha$={FIXED_ALPHA} + LEARNED $\varphi$ "
                 rf"(radius err={EVAL_RADIUS_ERROR}, bias={EVAL_BIAS_MAGNITUDE}$\uparrow$)",
                 fontsize=18, y=1.005)
    ...
    for i, data in enumerate(all_scenarios):
        ...
        ax.text(5, -12, f"TRUE r={TRUE_RADIUS}, EST r={EVAL_ESTIMATED_RADIUS}\n"
                        ...
                        f"Bias={EVAL_BIAS_MAGNITUDE} upward",
        ...
        sc = ax.scatter(phi_x[:-1:step_skip], phi_y[:-1:step_skip],
                        c=r["phis"][::step_skip], cmap="YlOrRd",
                        vmin=0.0, vmax=10.0, s=14, zorder=5, label=r"FIXED $\alpha$ + LEARNED $\varphi$")
        cbar = plt.colorbar(sc, ax=ax, fraction=0.046, pad=0.04)
        cbar.set_label(r"$\varphi$")
        ...
        # --- Column 2: Phi over time with Obstacle Zones (alpha is constant line) ---
        ax = axs[i, 1]
        ...
        for oi in range(3):
            ...
            ax.text(t_min, 10.3, f"{OBS_LABELS[oi]}", fontsize=6,
                    color=OBS_COLORS[oi], ha="center", va="top", fontweight="bold")

        ax.plot(r["phis"], color="darkorange", linewidth=2.5, label=r"$\varphi$ (learned)", zorder=5)
        ax.axhline(FIXED_ALPHA, color="purple", linewidth=2, linestyle="--",
                   label=rf"$\alpha$={FIXED_ALPHA} (fixed)", zorder=4)
        ax.set_ylabel(r"$\varphi$ / $\alpha$ Value")
        ax.set_ylim(-0.5, 11.0)

        ax.set_title(rf"{scen['name']} -- FIXED $\alpha$={FIXED_ALPHA} + LEARNED $\varphi$", fontsize=11)
        ...
        # --- Column 4: Fixed Alpha line + Per-Obstacle Distance (TRUE) ---
        ax = axs[i, 3]
        ax.set_ylabel(r"$\alpha$ Value (FIXED)", color="purple")
        ax.axhline(FIXED_ALPHA, color="purple", linewidth=2.5, linestyle="--",
                   label=rf"$\alpha$={FIXED_ALPHA} (fixed)", zorder=5)
        ...
        ax.set_title(rf"{scen['name']} -- Fixed $\alpha$ vs Obstacle", fontsize=11)
        ax.set_xlabel("Time Step")
        ax.grid(True, alpha=0.3)
        if i == 0:
        ...
        # --- Column 5: Phi + Per-Obstacle Distance (TRUE) — KEY column ---
        ax = axs[i, 4]
        ...
        ax.set_title(rf"{scen['name']} -- $\varphi$ vs Obstacle (KEY)", fontsize=11)
        ax.set_xlabel("Time Step")
        ax.grid(True, alpha=0.3)
        if i == 0:
            lines1, labels1 = ax.get_legend_handles_labels()
        ...
        # --- Column 6: Policy Map — phi vs TRUE distance (alpha is constant) ---
        ax = axs[i, 5]
        min_dists_per_t = np.array(r["dist"])
        phis_arr = np.array(r["phis"])

        ax.scatter(min_dists_per_t, phis_arr,
                   c="darkorange", s=12, alpha=0.5, marker="^", zorder=3, label=r"$\varphi$")
        ...
        ax.set_title(rf"{scen['name']} -- $\varphi$ vs Distance", fontsize=11)
        ax.grid(True, alpha=0.3)
        ax.legend(loc="upper right", fontsize=7)
    ...
    for idx, scen in enumerate(random_scenarios):
        ...
        result = run_episode(env, model, scen, scen["radius_error"], bias=scen["bias"])
    ...
    fig.suptitle(rf"Aggregate -- FIXED $\alpha$={FIXED_ALPHA} + LEARNED $\varphi$ "
                 rf"(radius err$\sim$U(-1,0), bias$\leq${EVAL_BIAS_MAGNITUDE}): "
                 rf"{N_RANDOM_SCENARIOS} Random Scenarios",
                 fontsize=16)
    ...
    ax.set_xticks([0]); ax.set_xticklabels([rf"FIXED $\alpha$ + $\varphi$"], fontsize=9)
    ax.set_ylim(0, 110)
    ...
    print(f"HAND-PICKED SCENARIOS (radius error={EVAL_RADIUS_ERROR}, est={EVAL_ESTIMATED_RADIUS}, "
          f"bias={EVAL_BIAS_MAGNITUDE} upward, FIXED alpha={FIXED_ALPHA})")
    print(f"{'='*80}")
    print(f"{'Scenario':<20} {'Reached':>8} {'Collided':>9} {'TrueMinD':>9} "
          f"{'Steps':>7} {'AvgSpd':>8} {'PathEff':>9} {'AvgPhi':>8}")
    print(f"{'-'*80}")

    for data in all_scenarios:
        scen, r = data["scen"], data["result"]
        avg_phi = np.mean(r["phis"]) if r["phis"] else 0.0

...
Experiment 28: Fixed alpha=1.0, learned phi only.
  - Policy controls [kx, ky, phi] — alpha is FIXED
  - Forces the policy to use phi as its only safety tuning knob
  - Includes radius noise + constant bias + randomized obstacles

Usage:
    cd experiments/28_fixed_alpha_learned_phi
    python run_all.py
"""
import subprocess
import sys
...
SCRIPTS = [
    ("Train fixed-alpha model (kx, ky, phi)", [sys.executable, "train_dynamic.py"]),

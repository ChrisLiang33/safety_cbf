"""
Exp 41: ISSf-CBF with raised phi minimum [0.5, 10.0] + reduced progress weight (25.0).
Agent controls [kx, ky, alpha, phi].

Key changes from baseline:
  - phi range [0.5, 10.0] instead of [0.01, 10.0] — always some buffer
  - progress reward * 25.0 instead of * 50.0 — less pressure to go fast
...
class PhiMinRebalanceEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        for i in range(3):
            error = self.np_random.uniform(self.radius_error_range[0], self.radius_error_range[1])
            self.estimated_radius[i] = max(self.true_radius[i] + error, 1.0)
        bias_mag = self.np_random.uniform(self.bias_magnitude_range[0], self.bias_magnitude_range[1])
    ...
    def step(self, action):
        ...
        if min_obs_dist_true < 0:
            ...
            reward = progress * 25.0 - 1.0  # REDUCED progress weight
        ...
        return self._get_obs(), float(reward), bool(terminated), False, {
            ...
            "estimated_radius": list(self.estimated_radius), "bias": self.bias.copy(),
        }

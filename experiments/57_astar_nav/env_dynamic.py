"""
Exp 57: A* navigation → train alpha/phi with SHIELD SDF CBF.
Agent controls [alpha, phi] ONLY.
Navigation comes from A* path planner (classical, not learned).

Architecture mirrors SHIELD paper:
  A* planner → learned adaptive CBF (alpha, phi) → robot

A* planner:
  - Discretizes environment into occupancy grid
  - Inflates obstacles by estimated radius
  - Plans shortest path from robot to target
  - Waypoint following with proportional controller gives kx, ky
  - Replans every REPLAN_INTERVAL steps or when path is exhausted
...
class AStarNavEnv(gym.Env):
    def __init__(self, systematic_bias_range=(-0.6, 0.4), jitter_range=(-0.2, 0.2),
        ...
        # ACTION: [alpha, phi] only — kx, ky from A* planner

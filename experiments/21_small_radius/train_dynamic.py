...
if __name__ == "__main__":
    print("Training phi-CBF + HIGH noise + SMALL radius model (kx, ky, alpha, phi)...")
    print("  ISSf-CBF: Lgh @ u >= -alpha * h(x) + (||Lgh||^2 * phi) / h(x)")
    print("  Domain randomization: noise scale ~ U(0.0, 0.5) per episode")

    env_fns = [lambda: PhiCBFSmallRadiusEnv(disturbance_range=(0.0, 0.5)) for _ in range(N_ENVS)]

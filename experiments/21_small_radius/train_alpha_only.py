...
if __name__ == "__main__":
    print("Training alpha-only + HIGH noise + SMALL radius model (kx, ky, alpha)...")
    print("  Standard CBF: Lgh @ u >= -alpha * h(x)")
    print("  Domain randomization: noise scale ~ U(0.0, 0.5) per episode")

    env_fns = [lambda: AlphaOnlySmallRadiusEnv(disturbance_range=(0.0, 0.5)) for _ in range(N_ENVS)]

"""
3-obstacle weave with learnable phi (ISSf-CBF) + HIGH disturbance + SMALL radius obstacles.
Agent controls [kx, ky, alpha, phi]. Large noise added to dynamics.

ISSf-CBF constraint: Lgh @ u >= -alpha * h(x) + (||Lgh||^2 * phi) / h(x)

Training noise range [0.0, 0.5] — 5x higher than exp 15.
Goal: force the agent to learn meaningful phi to compensate for large noise.
"""
...
class PhiCBFSmallRadiusEnv(gym.Env):

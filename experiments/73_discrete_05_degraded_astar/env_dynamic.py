"""
Exp 73: A* with DEGRADED planning + DISCRETE alpha/phi (step=0.5).

Key idea: A* handles navigation but is deliberately weakened so the CBF
must provide safety:
  - REPLAN_INTERVAL = 30 (replans every 3 seconds, not every 1 second)
  - INFLATION_MARGIN = 0.0 (A* plans paths right along obstacle edges)

This means A* gives a reasonable direction but does NOT guarantee safety.
The robot will follow stale paths that cut close to obstacles, especially
with moving obstacles. The CBF (alpha/phi) must compensate.

Discrete actions with step=0.5 for visible reactive patterns.
Target behavior: alpha drops + phi rises near obstacles, reverts after.
...
class DiscreteAStarDegradedEnv(gym.Env):
    def __init__(self, systematic_bias_range=(-0.6, 0.4), jitter_range=(-0.2, 0.2),
        ...
        # ACTION: Discrete(9) = 3 alpha choices x 3 phi choices
    ...
    def _build_occupancy_grid(self):
        ...
        for i in range(3):
            # No inflation — A* plans right along obstacle edges

"""
Exp 60: A* navigation + 5 obstacles → train alpha/phi.
Agent controls [alpha, phi] ONLY.

Based on Exp 57 with more obstacles:
  5 obstacles instead of 3, spread across 5 x-bands.
  More encounters per episode = more training signal.
  Observation space expanded to include 5 obstacles.

The A* planner WILL route through tight spaces if that's shortest.
This guarantees consistent near-obstacle training signal for alpha/phi.
...
GRID_X_RANGE = (-5, 165)
GRID_Y_RANGE = (-22, 22)
INFLATION_MARGIN = 1.0  # extra margin beyond estimated radius
K_PROPORTIONAL = 4.0    # proportional gain for waypoint following
WAYPOINT_THRESHOLD = 3.0  # meters, advance to next waypoint
REPLAN_INTERVAL = 50     # steps between replans


N_OBS = 5


class AStarMoreObsEnv(gym.Env):
    def __init__(self, systematic_bias_range=(-0.6, 0.4), jitter_range=(-0.2, 0.2),
        ...
        self.true_radius = [5.0] * N_OBS
        self.estimated_radius = [5.0] * N_OBS
        ...
        # ACTION: [alpha, phi] only
        self.action_space = spaces.Box(
            low=np.array([0.1, 0.01], dtype=np.float32),
            high=np.array([5.0, 10.0], dtype=np.float32),
            dtype=np.float32,
        )
        # 5 obstacles x 3 (rel_x, rel_y, est_radius) + target x 3 + velocity x 2 = 20
        self.observation_space = spaces.Box(
            low=np.array([-180, -35, 0] * N_OBS + [-180, -35, 0.1, -7, -7], dtype=np.float32),
            high=np.array([180, 35, 10] * N_OBS + [180, 35, 5.0, 7, 7], dtype=np.float32),
            dtype=np.float32,
        )

        self.robot_pos = np.zeros(2)
        self.velocity = np.zeros(2)
        self.obs_pos = [np.zeros(2) for _ in range(N_OBS)]
        ...
        # QP with single constraint (SHIELD SDF)
    ...
    def _get_nav_command(self):
        """Get kx, ky from A* waypoint following."""
        if self.path_idx >= len(self.path):
            # Path exhausted — go direct to target
    ...
    def step(self, action):
        ...
        dist2obs_true = [np.linalg.norm(self.robot_pos - self.obs_pos[i]) - self.true_radius[i] for i in range(N_OBS)]

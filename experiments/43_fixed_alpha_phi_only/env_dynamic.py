"""
Exp 43: FIXED alpha + learned phi only.
Agent controls [kx, ky, phi]. Alpha is fixed at 1.0.

Key idea: force the policy to use phi as its ONLY safety tuning knob.
If phi learns adaptive behavior here, it confirms phi CAN be learned
when alpha isn't available as an alternative.
...
class FixedAlphaPhiOnlyEnv(gym.Env):

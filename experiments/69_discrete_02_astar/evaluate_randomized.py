"""
Exp 69: Discrete A* + MOVING obstacles -- fully randomized eval.
Action is Discrete(9): 3 alpha x 3 phi (down/stay/up). 3 circular obstacles that move.
...
from env_dynamic import DiscreteAStarMovingObsEnv, ALPHA_INIT, PHI_INIT
...
# Discrete action colors: 0=down(red), 1=stay(gray), 2=up(blue)
...
def run_episode(env, model, scen):
    ...
    # Track obstacle positions over time
...
def plot_discrete_actions(ax_alpha, ax_phi, scen, r, fontsize_title=12, fontsize_label=11):
    """Plot discrete action choices over time as colored scatter."""
...
if __name__ == "__main__":
    ...
    # --- COMBINED: trajectory + alpha/phi + discrete actions (3 columns) ---
    n_scen = len(scenarios)
    fig, axs = plt.subplots(n_scen, 3, figsize=(36, 7 * n_scen),
                            gridspec_kw={"width_ratios": [1.4, 1, 0.6]})
    fig.suptitle("Exp 69: Discrete A* Moving Obs (step=0.2)",
                 fontsize=18, y=1.005)
    ...
    for i, data in enumerate(all_scenarios):
        ...
        # Column 3: discrete actions — split into two subplots using inset axes
        # We use the third column axis for alpha actions (top half)
        # and create a twin below for phi actions
        ax3 = axs[i, 2]
        ax3.set_visible(False)
        # Create two sub-axes within the third column's bounding box
        bbox = ax3.get_position()
        ax_alpha_act = fig.add_axes([bbox.x0, bbox.y0 + bbox.height * 0.55,
                                     bbox.width, bbox.height * 0.4])
        ax_phi_act = fig.add_axes([bbox.x0, bbox.y0 + bbox.height * 0.05,
                                   bbox.width, bbox.height * 0.4])
    ...
    for i, data in enumerate(all_scenarios):
        ...
        # Discrete actions: two stacked subplots in the third column

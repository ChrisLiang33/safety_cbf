"""
Exp 69: A* navigation + MOVING obstacles + DISCRETE alpha/phi actions.
Agent chooses from 9 discrete actions (3 alpha x 3 phi): up/stay/down.

Key differences from exp 61:
  - Discrete(9) action space instead of continuous Box
  - alpha and phi are persistent state variables (init 2.5, 1.0)
  - Each step modifies alpha/phi by +/- ALPHA_STEP/PHI_STEP or stays
  - Observation includes current alpha and phi (16 dims total)
...
class DiscreteAStarMovingObsEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # Decode discrete action into alpha/phi adjustments
        alpha_action = action // 3   # 0=down, 1=stay, 2=up
        phi_action = action % 3      # 0=down, 1=stay, 2=up
        alpha_delta = (alpha_action - 1) * ALPHA_STEP  # -0.2, 0, +0.2
        phi_delta = (phi_action - 1) * PHI_STEP        # -0.2, 0, +0.2

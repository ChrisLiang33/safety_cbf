"""
ISSf-CBF with alpha + phi, ONLY constant bias disturbance (no radius noise).
Agent controls [kx, ky, alpha, phi].

ISSf-CBF constraint: Lgh @ u >= -alpha * h(x) + (||Lgh||^2 * phi) / h(x)
Dynamics: robot_pos += safe_u * dt + bias * dt

Isolates the dynamics uncertainty: the CBF uses correct obstacle radius,
but the robot gets pushed by an unmodeled persistent force.
Phi should learn to add margin to account for this force.
...
class ISSfBiasOnlyEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # Dynamics with bias (no radius noise)

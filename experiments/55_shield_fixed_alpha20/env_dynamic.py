"""
Exp 55: SHIELD SDF + fixed alpha=2.0 + collision -250.
Agent controls [kx, ky, phi]. Alpha is FIXED at 2.0.

Same as Exp 53 (SHIELD SDF pipeline Eq 19-22) but with fixed alpha
and higher collision penalty. Compare to Exp 50 (squared CBF + fixed
alpha=2.0 + collision -250) — our best phi result so far.

This is the key experiment: does the SHIELD SDF formulation improve
phi learning compared to the squared CBF, when alpha is fixed?
...
class ShieldFixedAlpha20Env(gym.Env):

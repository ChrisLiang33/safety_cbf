...
Experiment 34: Level 4 -- ISSf-CBF with alpha + phi + bias + radius noise.
  - ISSf-CBF constraint: Lgh @ u >= -alpha * h(x) + (||Lgh||^2 * phi) / h(x)
  - Constant bias + radius estimation error
  - Does phi provide adaptive robustness under combined uncertainty?

Usage:
    cd experiments/34_level4_issf_cbf_full
    python run_all.py
"""
import subprocess
import sys
...
SCRIPTS = [
    ("Train Level 4 ISSf-CBF (kx, ky, alpha, phi)", [sys.executable, "train_dynamic.py"]),

"""
Level 4: Navigation + ISSf-CBF with alpha + phi + disturbance + radius noise.
Agent controls [kx, ky, alpha, phi]. QP enforces safety with robustness margin.

ISSf-CBF constraint: Lgh @ u >= -alpha * h(x) + (||Lgh||^2 * phi) / h(x)
Dynamics: robot_pos += safe_u * dt + bias * dt

Combined uncertainty:
  1. DYNAMICS: constant bias (wind/external force), sampled once per episode
  2. PERCEPTION: radius estimation error, sampled once per episode

The phi term provides formal robustness margin to handle both sources of error.
This is the full ISSf-CBF — the phi should finally be essential here since:
  - The CBF uses wrong radius (perception error)
  - The robot gets pushed by unmodeled forces (dynamics error)
  - Without phi, the standard CBF can't account for either
"""
...
class Level4ISSfCBFEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        # TRUE radius for collision
        self.true_radius = [5.0, 5.0, 5.0]

        # ESTIMATED radius for CBF + observation
        for i in range(3):

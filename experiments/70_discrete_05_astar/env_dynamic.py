"""
Exp 70: Discrete A* navigation + MOVING obstacles (step=0.5).

Nearly identical to exp 69 but with larger step size (0.5 vs 0.2).
Agent picks from Discrete(9) actions: 3 alpha choices x 3 phi choices.

Action mapping:
  alpha_action = action // 3   # 0=down, 1=stay, 2=up
  phi_action   = action % 3    # 0=down, 1=stay, 2=up
  alpha_delta  = (alpha_action - 1) * ALPHA_STEP
  phi_delta    = (phi_action   - 1) * PHI_STEP

Persistent state: self.alpha (init=2.5), self.phi (init=1.0), updated per step.
Observation: 16 dims (14 from exp 61 + current alpha, phi).

ALPHA_STEP = 0.5, PHI_STEP = 0.5
~10 steps to traverse alpha's full range [0.1, 5.0].

SHIELD SDF pipeline (Eq 19-22) with single constraint.
Moving obstacles, A* REPLAN_INTERVAL=10.
"""
...
class DiscreteAStarMovingObsEnv05(gym.Env):
    def __init__(self, systematic_bias_range=(-0.6, 0.4), jitter_range=(-0.2, 0.2),
        ...
        # ACTION: Discrete(9) = 3 alpha x 3 phi
        self.action_space = spaces.Discrete(9)

        # OBSERVATION: 14 from exp 61 + alpha + phi = 16

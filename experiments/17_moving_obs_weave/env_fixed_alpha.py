"""
3-obstacle weave with MOVING obstacles, fixed alpha + proportional k_nom baseline.
CBF includes drift term from obstacle velocity.
"""
...
class FixedAlphaMovingObsEnv(gym.Env):
    def __init__(self, alpha=1.0, obs_speed_range=(0.1, 0.5), obs_drift_limit=3.0):
        super().__init__()
        self.dt = 0.1
        self.fixed_alpha = alpha
        self.k_nom_speed = 3.0
        self.obs_speed_range = obs_speed_range
        self.obs_drift_limit = obs_drift_limit
        ...
        self.obs_init_pos = [np.zeros(2) for _ in range(3)]

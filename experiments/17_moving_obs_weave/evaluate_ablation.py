"""
Moving-obstacle weave ablation: dynamic [kx,ky,alpha] vs fixed alpha.

Obstacles drift with bounded velocity. CBF includes Lf_h drift term.
...
from env_fixed_alpha import FixedAlphaMovingObsEnv
...
# Hand-picked scenarios with obstacle velocities
SCENARIOS = [
    {"name": "Standard Slalom",
     "obs": [np.array([30.0, 6.0]), np.array([50.0, -6.0]), np.array([70.0, 6.0])],
     "obs_vel": [np.array([0.0, -0.3]), np.array([0.0, 0.3]), np.array([0.0, -0.3])],
     "target_pos": np.array([100.0, 0.0]), "target_radius": 2.0},
    {"name": "Closing In",
     "obs": [np.array([30.0, 5.0]), np.array([50.0, -5.0]), np.array([70.0, 5.0])],
     "obs_vel": [np.array([0.0, -0.4]), np.array([0.0, 0.4]), np.array([0.0, -0.4])],
     "target_pos": np.array([100.0, 0.0]), "target_radius": 2.0},
    {"name": "Lateral Drift",
     "obs": [np.array([30.0, 6.0]), np.array([50.0, -6.0]), np.array([70.0, 6.0])],
     "obs_vel": [np.array([0.3, 0.0]), np.array([-0.3, 0.0]), np.array([0.3, 0.0])],
     "target_pos": np.array([100.0, 0.0]), "target_radius": 2.0},
    {"name": "Head-On Obs 1",
     "obs": [np.array([30.0, 3.0]), np.array([50.0, -6.0]), np.array([70.0, 6.0])],
     "obs_vel": [np.array([-0.4, -0.2]), np.array([0.0, 0.2]), np.array([0.0, -0.2])],
     "target_pos": np.array([100.0, 0.0]), "target_radius": 2.0},
    {"name": "All Drifting Down",
     "obs": [np.array([30.0, 8.0]), np.array([50.0, -4.0]), np.array([70.0, 8.0])],
     "obs_vel": [np.array([0.0, -0.3]), np.array([0.0, -0.3]), np.array([0.0, -0.3])],
     "target_pos": np.array([100.0, 0.0]), "target_radius": 2.0},
    {"name": "Scatter Motion",
     "obs": [np.array([25.0, 6.0]), np.array([50.0, -7.0]), np.array([75.0, 5.0])],
     "obs_vel": [np.array([0.2, -0.2]), np.array([-0.2, 0.3]), np.array([0.1, -0.3])],
     "target_pos": np.array([100.0, 0.0]), "target_radius": 2.0},
    {"name": "Fast Obstacles",
     "obs": [np.array([30.0, 6.0]), np.array([50.0, -6.0]), np.array([70.0, 6.0])],
     "obs_vel": [np.array([0.0, -0.5]), np.array([0.0, 0.5]), np.array([0.0, -0.5])],
     "target_pos": np.array([100.0, 0.0]), "target_radius": 2.0},
    {"name": "Slow Obstacles",
     "obs": [np.array([30.0, 6.0]), np.array([50.0, -6.0]), np.array([70.0, 6.0])],
     "obs_vel": [np.array([0.05, -0.1]), np.array([-0.05, 0.1]), np.array([0.05, -0.1])],
     "target_pos": np.array([100.0, 0.0]), "target_radius": 2.0},
]
...
def setup_scenario(env, scen):
    ...
    for i in range(3):
        ...
        env.obs_init_pos[i] = scen["obs"][i].copy()
        env.obs_vel[i] = scen["obs_vel"][i].copy()
...
def run_dynamic_episode(env, model, scen):
    ...
    dist_list = []
    per_obs_dists = [[], [], []]
    # Track obstacle positions over time for plotting
    obs_traj = [[[], []], [[], []], [[], []]]  # obs_traj[i] = [xs, ys]
    total_reward = 0.0
    ...
    for step in range(MAX_STEPS):
        ...
        for oi in range(3):
            obs_traj[oi][0].append(env.obs_pos[oi][0])
            obs_traj[oi][1].append(env.obs_pos[oi][1])
    ...
    return {
        ...
        "per_obs_dists": per_obs_dists, "obs_traj": obs_traj,
...
def generate_random_scenarios(n, seed=42):
    ...
    for i in range(n):
        ...
        obs_list, vel_list = [], []
        for x_lo, x_hi, y_lo, y_hi in [(20, 35, 5, 8), (45, 60, -8, -5), (65, 80, 5, 8)]:
            ox = rng.uniform(x_lo, x_hi)
            oy = rng.uniform(y_lo, y_hi)
            obs_list.append(np.array([ox, oy]))
            spd = rng.uniform(0.1, 0.5)
            ang = rng.uniform(0, 2 * np.pi)
            vel_list.append(np.array([spd * np.cos(ang), spd * np.sin(ang)]))
        scenarios.append({
            "name": f"Random_{i}",
            "obs": obs_list, "obs_vel": vel_list,
...
if __name__ == "__main__":
    ...
    print("Loading moving-obstacle weave model...")
    dyn_env = MovingObsWeaveDynamicEnv()
    dyn_model = PPO.load(DYNAMIC_MODEL_PATH)

    fixed_envs = {}
    for alpha in FIXED_ALPHAS:
        fixed_envs[alpha] = FixedAlphaMovingObsEnv(alpha=alpha)
    ...
    # Traj (w/ obs motion trails) | Speed | Alpha+Dist | Speed+Alpha | Policy Map
    # =====================================================================
    ...
    fig.suptitle(r"Moving-Obs Weave: Dynamic [kx,ky,$\alpha$] vs Fixed $\alpha$ + Prop. Control",
                 fontsize=18, y=1.005)

    for i, data in enumerate(all_scenarios):
        scen, dyn, fixed_results = data["scen"], data["dyn"], data["fixed"]

        # --- Column 1: Trajectory with obstacle motion trails ---
        ax = axs[i, 0]
        # Draw obstacle initial position (faded) and trail
        for j in range(3):
            ax.add_patch(plt.Circle(scen["obs"][j], OBS_RADIUS,
                                    color=OBS_COLORS[j], alpha=0.1, linestyle="--"))
            # Draw obstacle motion trail from dynamic episode
            if "obs_traj" in dyn:
                ot = dyn["obs_traj"][j]
                ax.plot(ot[0], ot[1], color=OBS_COLORS[j], linewidth=1,
                        linestyle=":", alpha=0.4)
                # Draw final obstacle position
                if len(ot[0]) > 0:
                    ax.add_patch(plt.Circle((ot[0][-1], ot[1][-1]), OBS_RADIUS,
                                            color=OBS_COLORS[j], alpha=0.2))
            # Velocity arrow from initial position
            v = scen["obs_vel"][j]
            ax.annotate("", xy=(scen["obs"][j][0] + v[0]*50,
                                scen["obs"][j][1] + v[1]*50),
                        xytext=(scen["obs"][j][0], scen["obs"][j][1]),
        ...
        ax.plot(dyn["k_nom_speeds"], color="teal", linewidth=1, linestyle=":",
                alpha=0.5, label=r"$\|k_{nom}\|$ (requested)")
        ...
        ax.set_title(f"{scen['name']} — Speed + Alpha", fontsize=11)
        ax.set_xlabel("Time Step")
        ax.grid(True, alpha=0.3)
        if i == 0:
            lines1, labels1 = ax.get_legend_handles_labels()
    ...
    fig.suptitle(f"Aggregate — Moving-Obs Weave ({N_RANDOM_SCENARIOS} Random Scenarios)",
                 fontsize=16)

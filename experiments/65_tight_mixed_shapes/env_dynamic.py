"""
Exp 65: A* + tighter mixed obstacle shapes
Agent controls [alpha, phi] ONLY.

Key change vs Exp 63: obstacles are more tightly spaced (5 obstacles in
narrower x-bands, y-range [-6,6]) so the A* path must go through narrow
gaps where the CBF must engage.
...
The observation space encodes "effective radius" (half the max dimension)
per obstacle. 5 obstacles x 3 + target x 3 + velocity x 2 = 20.
...
class AStarTightMixedShapesEnv(gym.Env):
    def __init__(self, systematic_bias_range=(-0.6, 0.4), jitter_range=(-0.2, 0.2),
        ...
        self.n_obs = 5  # 5 tightly spaced mixed shapes
        ...
        # 5 obstacles x 3 (rel_x, rel_y, effective_radius) + target x 3 + velocity x 2 = 20
    ...
    def _generate_obstacles(self):
        """Generate a mix of circles, rectangles, and line segments in tight bands."""
        obstacles = []
        x_bands = [(20.0, 40.0), (45.0, 65.0), (70.0, 90.0), (95.0, 115.0), (120.0, 140.0)]

        for x_low, x_high in x_bands:
            cx = self.np_random.uniform(x_low, x_high)
            cy = self.np_random.uniform(-6.0, 6.0)

"""
Exp 61: A* navigation + MOVING obstacles → train alpha/phi.
Agent controls [alpha, phi] ONLY.

Key change: obstacles move with constant velocity during the episode.
...
A* replans every 10 steps (more frequent than exp 57's 50)
to track moving obstacles.

This tests whether alpha/phi can learn to handle dynamic environments
where the obstacle positions change over time — the CBF must
continuously adapt, not just react to static geometry.
...
class AStarMovingObsEnv(gym.Env):

"""
Phi-CBF + VERY HIGH Disturbance ablation:
...
from env_dynamic import PhiCBFVeryHighNoiseEnv
from env_alpha_only import AlphaOnlyVeryHighNoiseEnv
...
if __name__ == "__main__":
    ...
    phi_env = PhiCBFVeryHighNoiseEnv()
    phi_model = PPO.load(PHI_MODEL_PATH)

    ao_env = AlphaOnlyVeryHighNoiseEnv()
    ...
    fig.suptitle(rf"VERY HIGH Noise (noise={EVAL_NOISE}): Dynamic $\alpha$+$\varphi$ (ISSf-CBF) vs Dynamic $\alpha$-only (standard CBF)",
                 fontsize=18, y=1.005)
    ...
    fig.suptitle(rf"Aggregate — VERY HIGH Noise (noise={EVAL_NOISE}): $\alpha$+$\varphi$ vs $\alpha$-only ({N_RANDOM_SCENARIOS} Random Scenarios)",
                 fontsize=16)

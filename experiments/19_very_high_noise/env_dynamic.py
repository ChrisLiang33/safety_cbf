"""
3-obstacle weave with learnable phi (ISSf-CBF) + VERY HIGH disturbance.
Agent controls [kx, ky, alpha, phi]. Large noise added to dynamics.

ISSf-CBF constraint: Lgh @ u >= -alpha * h(x) + (||Lgh||^2 * phi) / h(x)

Training noise range [0.0, 1.0] — 10x higher than exp 15, 2x higher than exp 18.
...
class PhiCBFVeryHighNoiseEnv(gym.Env):

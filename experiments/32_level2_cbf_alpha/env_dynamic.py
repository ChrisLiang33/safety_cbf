"""
Level 2: Navigation + CBF with learnable alpha.
Agent controls [kx, ky, alpha]. QP enforces safety.

Standard CBF constraint: Lgh @ u >= -alpha * h(x)

No disturbance, no radius noise. Clean CBF test.
Same gentler randomization as Level 1.

Expected: 0% collisions (CBF guarantees safety), high target success.
"""
...
class Level2CBFAlphaEnv(gym.Env):
    def __init__(self):
        super().__init__()
        self.dt = 0.1
        self.obs_radius = 5.0
        ...
        # OBS: 14 dims (same as previous experiments for consistency)

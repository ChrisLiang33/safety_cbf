"""
One-shot: train all models then evaluate (2-obstacle + CBF penalty).
...
SCRIPTS = [
    ("Train fixed-alpha baselines (CBF penalty)", [sys.executable, "train_fixed_alphas.py"]),
    ("Train dynamic-alpha (CBF penalty)", [sys.executable, "train_dynamic.py"]),
    ("Evaluate ablation (CBF penalty)", [sys.executable, "evaluate_ablation.py"]),
]

"""
3-obstacle weave with disturbance in robot dynamics.
Agent controls [kx, ky, alpha]. Noise is added to position update.

Domain randomization: disturbance scale is randomized each episode
so the agent learns a robust policy across noise levels.
"""
...
class DisturbanceWeaveDynamicEnv(gym.Env):
    def __init__(self, disturbance_range=(0.0, 0.1)):
        super().__init__()
        self.dt = 0.1
        self.k_nom_speed = 3.0
        self.disturbance_range = disturbance_range  # (min, max) for domain randomization
        self.disturbance_scale = 0.05  # current episode's noise magnitude
    ...
    def reset(self, seed=None, options=None):
        ...
        # Domain randomization: sample disturbance scale for this episode
    ...
    def step(self, action):
        ...
        # --- DISTURBANCE: additive noise on position update ---
        noise = self.np_random.uniform(
            -self.disturbance_scale, self.disturbance_scale, size=2
        )
        self.robot_pos += safe_u * self.dt + noise * self.dt
        self.velocity = safe_u.copy()  # velocity is the intended safe_u (before noise)

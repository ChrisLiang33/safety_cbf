"""
Exp 45: ISSf-CBF with higher max speed + systematic sensor error.
Agent controls [kx, ky, alpha, phi].

Based on Exp 44 (systematic sensor error) with key changes:
  - Max speed raised from 3 to 6 m/s
  - Larger environment (target at x=150) to give room for higher speeds
  - Obstacle bands spread wider. Max steps increased to 800.

This should make alpha's proximity-adaptive behavior visible:
  - Open area: robot can go 6 m/s, alpha high
  - Near obstacle: alpha drops, CBF constrains speed down
  - Speed contrast becomes visible

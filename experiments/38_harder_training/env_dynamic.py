"""
Exp 38: ISSf-CBF with HARDER training conditions.
Agent controls [kx, ky, alpha, phi].

Key changes:
  - Stronger bias: U(0.5, 1.5) — always significant, sometimes OOD
  - Larger radius error: U(-2.0, 0.0) — always underestimate (dangerous)
  - Random obstacle radius: U(3.0, 7.0)

The policy should experience more collisions during training without phi,
creating stronger learning signal to use phi adaptively.
"""
...
class HarderTrainingEnv(gym.Env):
    def __init__(self, radius_error_range=(-2.0, 0.0), bias_magnitude_range=(0.5, 1.5),
    ...
    def reset(self, seed=None, options=None):
        ...
        # ESTIMATED radius = true + error (ALWAYS underestimate)
        for i in range(3):

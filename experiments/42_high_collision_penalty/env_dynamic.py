"""
Exp 42: ISSf-CBF with higher collision penalty (-500).
Agent controls [kx, ky, alpha, phi].

Key change: collision penalty -500 instead of -100.
Stronger signal to avoid collisions → stronger signal to use phi for safety.
...
class HighCollisionPenaltyEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if min_obs_dist_true < 0:
            reward = -500.0  # HIGH COLLISION PENALTY
            terminated = True
        else:

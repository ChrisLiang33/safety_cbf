"""
Phi-CBF + CONSTANT BIAS ablation:
  Dynamic [kx,ky,alpha,phi] (ISSf-CBF) vs Dynamic [kx,ky,alpha] (standard CBF)
Both trained and evaluated with constant bias (eval magnitude=0.7).

Key difference: bias is sampled ONCE per episode (random direction, fixed magnitude)
and applied identically every timestep. This simulates sensor bias / systematic error.
...
from env_alpha_only import AlphaOnlyConstantBiasEnv
...
if __name__ == "__main__":
    ...
    phi_env = PhiCBFConstantBiasEnv()
    phi_model = PPO.load(PHI_MODEL_PATH)

    ao_env = AlphaOnlyConstantBiasEnv()
    ...
    fig.suptitle(rf"CONSTANT BIAS (magnitude={EVAL_BIAS_MAGNITUDE}, dir=$\uparrow$): Dynamic $\alpha$+$\varphi$ (ISSf-CBF) vs Dynamic $\alpha$-only (standard CBF)",
                 fontsize=18, y=1.005)
    ...
    fig.suptitle(rf"Aggregate -- CONSTANT BIAS (magnitude={EVAL_BIAS_MAGNITUDE}): $\alpha$+$\varphi$ vs $\alpha$-only ({N_RANDOM_SCENARIOS} Random Scenarios)",
                 fontsize=16)
    ...
    # =====================================================================
    print(f"\n{'='*100}")
    print(f"HAND-PICKED SCENARIO RESULTS (constant bias magnitude={EVAL_BIAS_MAGNITUDE}, dir=up)")
    print(f"{'='*100}")

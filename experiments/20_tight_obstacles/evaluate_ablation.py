"""
Phi-CBF + TIGHT Obstacles ablation:
...
SCENARIOS = [
    {"name": "Standard",
     "obs": [np.array([28.0, 4.0]), np.array([45.0, -4.0]), np.array([60.0, 4.0])],
     "target_pos": np.array([100.0, 0.0]), "target_radius": 2.0},
    {"name": "Tight",
     "obs": [np.array([28.0, 3.5]), np.array([45.0, -3.5]), np.array([60.0, 3.5])],
     "target_pos": np.array([100.0, 0.0]), "target_radius": 2.0},
    {"name": "Wide",
     "obs": [np.array([28.0, 5.0]), np.array([45.0, -5.0]), np.array([60.0, 5.0])],
     "target_pos": np.array([100.0, 0.0]), "target_radius": 2.0},
    {"name": "Clustered",
     "obs": [np.array([25.0, 4.0]), np.array([35.0, -4.0]), np.array([45.0, 4.0])],
     "target_pos": np.array([100.0, 0.0]), "target_radius": 2.0},
    {"name": "Spread",
     "obs": [np.array([25.0, 5.0]), np.array([45.0, -5.0]), np.array([65.0, 5.0])],
     "target_pos": np.array([100.0, 0.0]), "target_radius": 2.0},
    {"name": "Off-Center Target",
     "obs": [np.array([28.0, 4.0]), np.array([45.0, -4.0]), np.array([60.0, 4.0])],
     "target_pos": np.array([100.0, 3.0]), "target_radius": 2.0},
    {"name": "All Same Side",
     "obs": [np.array([25.0, 4.0]), np.array([45.0, 5.0]), np.array([65.0, 3.5])],
     "target_pos": np.array([100.0, 0.0]), "target_radius": 2.0},
    {"name": "Diagonal",
     "obs": [np.array([25.0, 5.0]), np.array([50.0, -3.5]), np.array([65.0, 4.0])],
...
def generate_random_scenarios(n, seed=42):
    ...
    for i in range(n):
        ...
        x1 = rng.uniform(25.0, 30.0)
        y1 = rng.uniform(3.5, 5.0)
        x2 = rng.uniform(40.0, 50.0)
        y2 = rng.uniform(-5.0, -3.5)
        x3 = rng.uniform(55.0, 65.0)
        y3 = rng.uniform(3.5, 5.0)
        scenarios.append({
            "name": f"Random_{i}",
...
if __name__ == "__main__":
    ...
    phi_env = PhiCBFTightObsEnv()
    phi_model = PPO.load(PHI_MODEL_PATH)

    ao_env = AlphaOnlyTightObsEnv()
    ...
    fig.suptitle(rf"TIGHT Obstacles (noise={EVAL_NOISE}): Dynamic $\alpha$+$\varphi$ (ISSf-CBF) vs Dynamic $\alpha$-only (standard CBF)",
                 fontsize=18, y=1.005)
    ...
    fig.suptitle(rf"Aggregate — TIGHT Obstacles (noise={EVAL_NOISE}): $\alpha$+$\varphi$ vs $\alpha$-only ({N_RANDOM_SCENARIOS} Random Scenarios)",
                 fontsize=16)

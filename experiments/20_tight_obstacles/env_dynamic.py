"""
3-obstacle weave with learnable phi (ISSf-CBF) + TIGHT obstacle spacing.
Agent controls [kx, ky, alpha, phi]. Large noise added to dynamics.

ISSf-CBF constraint: Lgh @ u >= -alpha * h(x) + (||Lgh||^2 * phi) / h(x)

Training noise range [0.0, 0.5] — same as exp 18.
Obstacle positions are tighter: narrower gaps force the robot through
challenging passages, making phi's safety margin more valuable.
"""
...
class PhiCBFTightObsEnv(gym.Env):
    def __init__(self, disturbance_range=(0.0, 0.5)):
        super().__init__()
        self.dt = 0.1
    ...
    def reset(self, seed=None, options=None):
        ...
        # Domain randomization: noise scale from [0.0, 0.5]
        ...
        x1 = self.np_random.uniform(25.0, 30.0)
        y1 = self.np_random.uniform(3.5, 5.0)
        self.obs_pos[0] = np.array([x1, y1])
        self.obs_radius[0] = 5.0

        x2 = self.np_random.uniform(40.0, 50.0)
        y2 = self.np_random.uniform(-5.0, -3.5)
        self.obs_pos[1] = np.array([x2, y2])
        self.obs_radius[1] = 5.0

        x3 = self.np_random.uniform(55.0, 65.0)
        y3 = self.np_random.uniform(3.5, 5.0)
    ...
    def step(self, action):
        ...
        return self._get_obs(), float(reward), bool(terminated), False, {
            ...
            "noise": noise, "disturbance_scale": self.disturbance_scale,
        }

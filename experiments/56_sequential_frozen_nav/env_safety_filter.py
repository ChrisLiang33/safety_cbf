"""
Exp 56 Phase 2: Safety filter environment.
Frozen navigation policy provides kx, ky.
Agent controls [alpha, phi] ONLY — the safety parameters.

The frozen nav policy is loaded at init and called every step to get
kx, ky. These go through the SHIELD SDF CBF QP with the learned
alpha/phi. The nav policy CANNOT adapt — it will drive the robot
near obstacles, forcing alpha/phi to learn.

SHIELD SDF pipeline (Eq 19-22) with single constraint (closest obs).
"""
...
class SafetyFilterEnv(gym.Env):
    def __init__(self, nav_model_path="./models_dynamic/nav_only_model",
    ...
    def step(self, action):
        ...
        # Get kx, ky from FROZEN navigation policy
        obs = self._get_obs()

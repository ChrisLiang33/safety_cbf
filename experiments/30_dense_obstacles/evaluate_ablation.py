"""
ISSf-CBF evaluation with 6 DENSE OBSTACLES + RADIUS ESTIMATION NOISE + CONSTANT BIAS.
No alpha-only baseline — focus on alpha/phi behavior analysis.

Eval: worst-case radius error = -1.0, bias magnitude = 0.7.
...
EVAL_ESTIMATED_RADIUS = TRUE_RADIUS + EVAL_RADIUS_ERROR
EVAL_BIAS_MAGNITUDE = 0.7

PHI_COLOR = "black"

OBS_COLORS = ["#e74c3c", "#e67e22", "#9b59b6", "#2ecc71", "#3498db", "#e91e63"]
OBS_LABELS = ["Obs 1", "Obs 2", "Obs 3", "Obs 4", "Obs 5", "Obs 6"]

SCENARIOS = [
    {"name": "Standard Grid",
     "obs": [np.array([25.0, 5.0]), np.array([25.0, -5.0]), np.array([50.0, 8.0]),
             np.array([50.0, -2.0]), np.array([75.0, 5.0]), np.array([75.0, -5.0])],
     "target_pos": np.array([100.0, 0.0]), "target_radius": 2.0},
    {"name": "Dense Corridor",
     "obs": [np.array([20.0, 5.0]), np.array([32.0, -5.0]), np.array([44.0, 5.0]),
             np.array([56.0, -5.0]), np.array([68.0, 5.0]), np.array([80.0, -5.0])],
     "target_pos": np.array([100.0, 0.0]), "target_radius": 2.0},
    {"name": "Clustered Left",
     "obs": [np.array([20.0, 4.0]), np.array([28.0, -4.0]), np.array([36.0, 3.0]),
             np.array([44.0, -3.0]), np.array([65.0, 6.0]), np.array([85.0, -6.0])],
     "target_pos": np.array([100.0, 0.0]), "target_radius": 2.0},
    {"name": "Wall with Gap",
     "obs": [np.array([50.0, 12.0]), np.array([50.0, 6.0]), np.array([50.0, -6.0]),
             np.array([25.0, -3.0]), np.array([75.0, 4.0]), np.array([80.0, -7.0])],
     "target_pos": np.array([100.0, 0.0]), "target_radius": 2.0},
    {"name": "Zigzag",
     "obs": [np.array([18.0, 6.0]), np.array([30.0, -6.0]), np.array([42.0, 6.0]),
             np.array([54.0, -6.0]), np.array([66.0, 6.0]), np.array([78.0, -6.0])],
     "target_pos": np.array([100.0, 0.0]), "target_radius": 2.0},
    {"name": "All Upper",
     "obs": [np.array([20.0, 5.0]), np.array([35.0, 8.0]), np.array([50.0, 4.0]),
             np.array([60.0, 9.0]), np.array([72.0, 6.0]), np.array([85.0, 7.0])],
     "target_pos": np.array([100.0, 0.0]), "target_radius": 2.0},
    {"name": "Tunnel",
     "obs": [np.array([30.0, 7.0]), np.array([30.0, -7.0]), np.array([50.0, 7.0]),
             np.array([50.0, -7.0]), np.array([70.0, 7.0]), np.array([70.0, -7.0])],
     "target_pos": np.array([100.0, 0.0]), "target_radius": 2.0},
    {"name": "Random Dense",
     "obs": [np.array([22.0, -3.0]), np.array([38.0, 7.0]), np.array([45.0, -8.0]),
             np.array([58.0, 2.0]), np.array([72.0, -5.0]), np.array([83.0, 6.0])],
     "target_pos": np.array([100.0, -2.0]), "target_radius": 2.0},
]


def setup_scenario(env, scen, radius_error, bias_angle):
    """Setup scenario with radius error and constant bias."""
    env.reset()
...
def run_episode(env, model, scen, radius_error, bias_angle):
    """Run episode with dynamic [kx, ky, alpha, phi] (ISSf-CBF)."""
    obs = setup_scenario(env, scen, radius_error, bias_angle)
    traj_x, traj_y, alphas, phis, speeds = [], [], [], [], []
    k_nom_speeds, safe_u_speeds = [], []
    dist_list_true = []
    per_obs_dists_true = [[] for _ in range(N_OBSTACLES)]
    total_reward = 0.0
...
def generate_random_scenarios(n, seed=42):
    """Generate random scenarios with random obstacle placement (6 obstacles)."""
...
if __name__ == "__main__":
    ...
    print(f"Loading model... (radius err={EVAL_RADIUS_ERROR}, bias mag={EVAL_BIAS_MAGNITUDE})")
    env = PhiCBFDenseObstaclesEnv()
    model = PPO.load(PHI_MODEL_PATH)

    # --- Hand-picked scenarios ---
    # Worst case: underestimate radius by 1.0 + upward bias
    hand_picked_bias_angle = np.pi / 2  # upward, toward upper obstacles
    print(f"\nRunning {len(SCENARIOS)} hand-picked scenarios...")
    all_scenarios = []
    for scen in SCENARIOS:
        result = run_episode(env, model, scen, EVAL_RADIUS_ERROR, hand_picked_bias_angle)
    ...
    fig.suptitle(rf"6 DENSE OBSTACLES + RADIUS NOISE + BIAS (true={TRUE_RADIUS}, est={EVAL_ESTIMATED_RADIUS}, "
                 rf"bias={EVAL_BIAS_MAGNITUDE}$\uparrow$): ISSf-CBF Analysis",
                 fontsize=18, y=1.005)
    ...
    for i, data in enumerate(all_scenarios):
        ...
        ax.annotate("", xy=(5, 4), xytext=(5, 0),
                    arrowprops=dict(arrowstyle="->", color="gray", lw=2))
        ax.text(7, 2, f"bias={EVAL_BIAS_MAGNITUDE}", fontsize=7, color="gray")

        ax.text(5, -12, f"TRUE r={TRUE_RADIUS}, EST r={EVAL_ESTIMATED_RADIUS}\n"
                        f"Gap={TRUE_RADIUS - EVAL_ESTIMATED_RADIUS}m + bias={EVAL_BIAS_MAGNITUDE}",
        ...
        ax.axvline(TRUE_RADIUS - EVAL_ESTIMATED_RADIUS, color="orange", linewidth=1,
                   linestyle="--", alpha=0.5, label=f"Danger zone")
    ...
    for idx, scen in enumerate(random_scenarios):
        ...
        result = run_episode(env, model, scen, scen["radius_error"], scen["bias_angle"])
    ...
    fig.suptitle(rf"Aggregate -- 6 DENSE OBSTACLES + RADIUS NOISE + BIAS: "
    ...
    # =====================================================================
    print(f"\n{'='*80}")
    print(f"HAND-PICKED SCENARIOS (radius err={EVAL_RADIUS_ERROR}, bias={EVAL_BIAS_MAGNITUDE} up)")
    print(f"{'='*80}")

"""
3-obstacle weave with learnable phi (ISSf-CBF) + CONSTANT BIAS disturbance.
Agent controls [kx, ky, alpha, phi]. Persistent directional bias per episode.

ISSf-CBF constraint: Lgh @ u >= -alpha * h(x) + (||Lgh||^2 * phi) / h(x)

Key difference from previous experiments: noise is sampled ONCE per episode and
held constant. This simulates sensor bias (systematic error) rather than random
jitter. The bias does NOT average out over time -- it persistently pushes the
robot in one direction, making the CBF robustness margin genuinely necessary.

Training: bias magnitude domain-randomized from [0.0, 1.0] per episode.
...
class PhiCBFHighEvalBiasEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        # Domain randomization: bias magnitude from [0.0, 1.0]
        ...
        # Random direction (unit vector) * magnitude
    ...
    def step(self, action):
        ...
        return self._get_obs(), float(reward), bool(terminated), False, {
            ...
            "bias": self.bias, "disturbance_scale": self.disturbance_scale,
        }

"""
Phi-CBF + HIGH EVAL BIAS (1.5) ablation:
  Dynamic [kx,ky,alpha,phi] (ISSf-CBF) vs Dynamic [kx,ky,alpha] (standard CBF)
Both trained with constant bias magnitude ~ U(0.0, 1.0).
Evaluated with constant bias magnitude = 1.5 (OUT OF DISTRIBUTION).

Key difference from exp 22: eval bias is 1.5 instead of 0.7, testing OOD generalization.
...
from env_dynamic import PhiCBFHighEvalBiasEnv
...
if __name__ == "__main__":
    ...
    print(f"Loading models... (eval bias magnitude={EVAL_BIAS_MAGNITUDE} -- OUT OF DISTRIBUTION)")
    phi_env = PhiCBFHighEvalBiasEnv()
    phi_model = PPO.load(PHI_MODEL_PATH)

    ao_env = AlphaOnlyHighEvalBiasEnv()
    ...
    fig.suptitle(rf"HIGH EVAL BIAS (1.5) (train $\leq$1.0, OOD): Dynamic $\alpha$+$\varphi$ (ISSf-CBF) vs Dynamic $\alpha$-only (standard CBF)",
                 fontsize=18, y=1.005)
    ...
    fig.suptitle(rf"Aggregate -- HIGH EVAL BIAS (1.5, OOD): $\alpha$+$\varphi$ vs $\alpha$-only ({N_RANDOM_SCENARIOS} Random Scenarios)",
                 fontsize=16)
    ...
    # =====================================================================
    print(f"\n{'='*100}")
    print(f"HAND-PICKED SCENARIO RESULTS (HIGH EVAL BIAS magnitude={EVAL_BIAS_MAGNITUDE}, OOD, dir=up)")
    print(f"{'='*100}")
    ...
    print(f"{'='*100}")
    print(f"AGGREGATE ({N_RANDOM_SCENARIOS} RANDOM SCENARIOS, random bias directions, OOD bias={EVAL_BIAS_MAGNITUDE})")
    print(f"{'='*100}")

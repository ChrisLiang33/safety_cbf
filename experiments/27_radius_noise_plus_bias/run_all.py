"""
One-shot: train ISSf-CBF model then evaluate.

Experiment 27: Radius estimation noise + constant bias + randomized obstacles.
  - No alpha-only baseline
  - Combined uncertainty: perception (radius error) + dynamics (constant bias)
  - Both sampled once per episode to avoid CLT averaging
  - Obstacles placed randomly (not forced slalom)

Usage:
    cd experiments/27_radius_noise_plus_bias
    python run_all.py
"""
import subprocess
import sys

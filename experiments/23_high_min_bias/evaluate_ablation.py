"""
Phi-CBF + HIGH MIN BIAS ablation:
  Dynamic [kx,ky,alpha,phi] (ISSf-CBF) vs Dynamic [kx,ky,alpha] (standard CBF)
Both trained with bias magnitude ~ U(0.5, 1.0) and evaluated with constant bias (eval magnitude=0.7).

Key difference from exp 22: training used higher minimum bias (0.5 vs 0.0),
forcing the policy to always face significant disturbance during training.
...
from env_dynamic import PhiCBFHighMinBiasEnv
...
def setup_scenario(env, scen, bias_angle):
    """Setup scenario with a specific constant bias direction."""
    env.reset()
    ...
    # Set constant bias for eval
    env.disturbance_scale = EVAL_BIAS_MAGNITUDE
...
def run_phi_episode(env, model, scen, bias_angle):
    """Run episode with dynamic [kx, ky, alpha, phi] (ISSf-CBF)."""
    obs = setup_scenario(env, scen, bias_angle)
...
def run_alpha_only_episode(env, model, scen, bias_angle):
...
if __name__ == "__main__":
    ...
    print(f"Loading models... (eval bias magnitude={EVAL_BIAS_MAGNITUDE})")
    phi_env = PhiCBFHighMinBiasEnv()
    phi_model = PPO.load(PHI_MODEL_PATH)

    ao_env = AlphaOnlyHighMinBiasEnv()
    ao_model = PPO.load(ALPHA_ONLY_MODEL_PATH)

    # --- Hand-picked scenarios ---
    # Use worst-case bias direction: pushing toward nearest obstacle
    # For slalom layout, bias pushing upward (+y) pushes toward obs 1 and 3
    print(f"\nRunning {len(SCENARIOS)} hand-picked scenarios...")
    hand_picked_bias_angle = np.pi / 2  # push upward toward obstacles
    all_scenarios = []
    for scen in SCENARIOS:
        phi_result = run_phi_episode(phi_env, phi_model, scen, hand_picked_bias_angle)
        ao_result = run_alpha_only_episode(ao_env, ao_model, scen, hand_picked_bias_angle)
    ...
    fig.suptitle(rf"HIGH MIN BIAS (train $\sim$ U(0.5,1.0), eval magnitude={EVAL_BIAS_MAGNITUDE}, dir=$\uparrow$): Dynamic $\alpha$+$\varphi$ (ISSf-CBF) vs Dynamic $\alpha$-only (standard CBF)",
                 fontsize=18, y=1.005)
    ...
    # =====================================================================
    # BATCH: 100 random scenarios (each with random bias direction)
    # =====================================================================
    ...
    for idx, scen in enumerate(random_scenarios):
        ...
        phi_r = run_phi_episode(phi_env, phi_model, scen, bias_angle)
        m_phi = methods[1]
        ...
        ao_r = run_alpha_only_episode(ao_env, ao_model, scen, bias_angle)
        m_ao = methods[0]
    ...
    fig.suptitle(rf"Aggregate -- HIGH MIN BIAS (train $\sim$ U(0.5,1.0), eval magnitude={EVAL_BIAS_MAGNITUDE}): $\alpha$+$\varphi$ vs $\alpha$-only ({N_RANDOM_SCENARIOS} Random Scenarios)",
                 fontsize=16)
    ...
    # =====================================================================
    print(f"\n{'='*100}")
    print(f"HAND-PICKED SCENARIO RESULTS (HIGH MIN BIAS, eval bias magnitude={EVAL_BIAS_MAGNITUDE}, dir=up)")
    print(f"{'='*100}")
    ...
    print(f"{'='*100}")
    print(f"AGGREGATE ({N_RANDOM_SCENARIOS} RANDOM SCENARIOS, random bias directions)")
    print(f"{'='*100}")

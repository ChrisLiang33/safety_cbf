"""
One-shot: train both models then evaluate (HIGH MIN BIAS variant).
  1. Train alpha-only model (standard CBF, no phi)
  2. Train alpha+phi model (ISSf-CBF)
  3. Evaluate ablation (compare both)

Same as exp 22 but training bias magnitude ~ U(0.5, 1.0) instead of U(0.0, 1.0).

Usage:
    cd experiments/23_high_min_bias
    python run_all.py
"""
import subprocess
import sys

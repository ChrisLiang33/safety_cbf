...
Use this as a gate before training the safety filter (Phase 2).

Usage:
    python eval_nav_success.py [--episodes 500] [--threshold 0.7]
"""
import argparse
import numpy as np
...
def run_eval(model_path, env_cls, n_episodes=500, seed=0):
    ...
    for ep in range(n_episodes):
        ...
        if dist2target < env.target_radius:
            ...
            # terminated but didn't reach target = collision or OOB
            collided += 1
        else:
            timeout += 1
...
if __name__ == "__main__":
    ...
    # Import the nav env — change this for different experiments
    ...
    if success_rate >= args.threshold:
        ...
        print(f"  Proceeding to Phase 2 (safety filter training).")
        exit(0)
    else:
        ...
        print(f"  Consider: more training steps, reward tuning, or environment adjustment.")
        exit(1)

"""
Exp 58 Phase 2: Hard safety filter environment.
Frozen EASY navigation policy provides kx, ky (learned with no obstacles).
Agent controls [alpha, phi] ONLY.

The frozen nav policy learned to beeline to the target — it has NO
obstacle avoidance behavior. When placed on a map with obstacles,
it will drive straight at them. The CBF MUST intervene, giving
alpha/phi a strong, consistent training signal.

SHIELD SDF pipeline (Eq 19-22) with single constraint.
Obstacles placed in tighter bands for more encounters.
"""
...
class SafetyHardEnv(gym.Env):
    def __init__(self, nav_model_path="./models_dynamic/nav_easy_model",
        ...
        # Load frozen easy navigation policy
    ...
    def _place_obstacles(self):
        """Place obstacles closer to the direct path (y near 0)
        to force more encounters with the beelining nav policy."""
        placed = []
        x_bands = [(20.0, 50.0), (60.0, 100.0), (110.0, 140.0)]
        for x_low, x_high in x_bands:
            x = self.np_random.uniform(x_low, x_high)
            # Tighter y-range: obstacles near the center where nav will drive
            y = self.np_random.uniform(-6.0, 6.0)
    ...
    def step(self, action):
        ...
        # Get kx, ky from FROZEN easy nav policy
        obs = self._get_obs()

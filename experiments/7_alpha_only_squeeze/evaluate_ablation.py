"""
Alpha-only squeeze gate ablation: fixed alpha vs learned alpha.
2 obstacles straddle the direct path — low alpha = crawl, high alpha = full speed.
...
from env_fixed_alpha import FixedAlpha2ObsEnv
...
# Hand-picked scenarios: obstacles straddle path at various gap widths
SCENARIOS = [
    {"name": "Very Tight (0.3m gap)",
     "obs1": np.array([5.0, 1.15]), "obs2": np.array([5.0, -1.15]),
     "target_pos": np.array([10.0, 0.0]), "target_radius": 1.0},
    {"name": "Tight (0.6m gap)",
     "obs1": np.array([5.0, 1.3]), "obs2": np.array([5.0, -1.3]),
     "target_pos": np.array([10.0, 0.0]), "target_radius": 1.0},
    {"name": "Medium (1.0m gap)",
     "obs1": np.array([5.0, 1.5]), "obs2": np.array([5.0, -1.5]),
     "target_pos": np.array([10.0, 0.0]), "target_radius": 1.0},
    {"name": "Wide (2.0m gap)",
     "obs1": np.array([5.0, 2.0]), "obs2": np.array([5.0, -2.0]),
     "target_pos": np.array([10.0, 0.0]), "target_radius": 1.0},
    {"name": "Early Gate",
     "obs1": np.array([3.0, 1.5]), "obs2": np.array([3.0, -1.5]),
     "target_pos": np.array([10.0, 0.0]), "target_radius": 1.0},
    {"name": "Late Gate",
     "obs1": np.array([7.0, 1.5]), "obs2": np.array([7.0, -1.5]),
     "target_pos": np.array([10.0, 0.0]), "target_radius": 1.0},
    {"name": "Off-Center Target",
     "obs1": np.array([5.0, 2.5]), "obs2": np.array([5.0, 0.5]),
     "target_pos": np.array([10.0, 2.0]), "target_radius": 0.8},
    {"name": "Staggered Gate",
     "obs1": np.array([4.0, 1.3]), "obs2": np.array([6.0, -1.3]),
     "target_pos": np.array([10.0, 0.0]), "target_radius": 1.0},
]
...
def generate_random_scenarios(n, seed=42):
    ...
    scenarios = []
    for i in range(n):
        target_y = rng.uniform(-1.5, 1.5)
        obs_x = rng.uniform(4.0, 6.0)
        path_y = target_y * obs_x / 10.0
        offset = rng.uniform(1.3, 2.0)
        target_radius = rng.uniform(0.5, 1.5)
        scenarios.append({
            "name": f"Random_{i}",
...
if __name__ == "__main__":
    ...
    print("Loading alpha-only squeeze gate model...")
    dyn_env = AlphaOnly2ObsDynamicEnv()
    dyn_model = PPO.load(DYNAMIC_MODEL_PATH)

    fixed_envs = {}
    for alpha in FIXED_ALPHAS:
        fixed_envs[alpha] = FixedAlpha2ObsEnv(alpha=alpha)
    ...
    fig, axs = plt.subplots(n_scen, 3, figsize=(26, 5 * n_scen))
    fig.suptitle(r"Alpha-Only Squeeze Gate: Fixed $\alpha$ vs Dynamic $\alpha$",
                 fontsize=18, y=1.005)
    ...
    for i, data in enumerate(all_scenarios):
        ...
        # --- Column 2: Robot Speed (||safe_u||) over time ---
        ax = axs[i, 1]
        for fa in FIXED_ALPHAS:
            r = fixed_results[fa]
        ...
        ax.axhline(2.0, color="gray", linewidth=0.8, linestyle=":", alpha=0.4, label="Max (2 m/s)")
        ...
        # --- Column 3: Alpha + Distance to nearest obstacle ---
        ax = axs[i, 2]
    ...
    fig.suptitle(f"Aggregate Metrics — Squeeze Gate ({N_RANDOM_SCENARIOS} Random Scenarios)",
                 fontsize=16)
    ...
    bars = ax.bar(x, vals, color=method_colors, alpha=0.8, edgecolor="black")
    ax.set_ylabel("Avg Steps to Complete")
    ...
    ax.set_xticks(x); ax.set_xticklabels(methods, fontsize=8)
    ax.axhline(2.0, color="gray", linewidth=1, linestyle="--", alpha=0.5, label="Max")
    for bar, val in zip(bars, vals):
        ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.02,
                f"{val:.2f}", ha="center", fontsize=9)

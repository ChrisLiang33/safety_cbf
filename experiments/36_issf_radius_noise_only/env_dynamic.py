"""
ISSf-CBF with alpha + phi, ONLY radius estimation noise (no bias).
Agent controls [kx, ky, alpha, phi].

ISSf-CBF constraint: Lgh @ u >= -alpha * h(x) + (||Lgh||^2 * phi) / h(x)
Dynamics: robot_pos += safe_u * dt (no disturbance)

Isolates the perception uncertainty: the robot moves perfectly, but the CBF
uses a wrong obstacle radius estimate. The phi term should learn to add
margin to compensate for the gap between estimated and true radius.

Radius error sampled ONCE per episode per obstacle (avoids CLT averaging).
Gentler x-band obstacle placement.
"""
...
class ISSfRadiusNoiseOnlyEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        # ESTIMATED radius for CBF + observation (sampled ONCE per episode)
        for i in range(3):

"""
ISSf-CBF with learnable alpha + phi, RADIUS NOISE ONLY (no bias).
Action is [kx, ky, alpha, phi]. QP enforces safety with robustness margin.

ISSf-CBF constraint: Lgh @ u >= -alpha * h(x) + (||Lgh||^2 * phi) / h(x)

Uncertainty: PERCEPTION only -- radius estimation error (obstacle appears smaller than it is).
...
def setup_scenario(env, scen, radius_error=None):
    env.reset()
...
def run_episode(env, model, scen, radius_error=None):
    obs = setup_scenario(env, scen, radius_error=radius_error)
    ...
    for step in range(MAX_STEPS):
        ...
        # TRUE distances for collision detection and plotting
...
if __name__ == "__main__":
    ...
    for scen in SCENARIOS:
        result = run_episode(env, model, scen, radius_error=EVAL_RADIUS_ERROR)
    ...
    fig.suptitle("ISSf-CBF RADIUS NOISE ONLY (no bias, error=-1.0)",
                 fontsize=18, y=1.005)
    ...
    for i, data in enumerate(all_scenarios):
        ...
        # --- Column 1: Trajectory with alpha colormap + true/estimated radius circles ---
        ax = axs[i, 0]
        for j in range(3):
    ...
    fig_agg.suptitle("ISSf-CBF Radius Noise Only: Alpha & Phi vs Distance to Nearest Obstacle (all scenarios)",
                     fontsize=14)
    ...
    # =====================================================================
    print(f"\n{'='*80}")
    print(f"HAND-PICKED SCENARIOS (no bias, radius_error={EVAL_RADIUS_ERROR})")
    print(f"{'='*80}")

"""
Exp 75, Phase 1: Learn alpha only (phi fixed at 0.5).

A* navigation with degraded planning, discrete alpha actions.
This is the first phase of sequential learning: alpha learns first,
then phi fine-tunes on top of the frozen alpha policy.

Key settings:
  - PHI_FIXED = 0.5 (constant, not learnable)
  - Action space: Discrete(3) — 0=alpha_down, 1=alpha_stay, 2=alpha_up
  - ALPHA_STEP = 0.5, ALPHA_INIT = 2.0
  - REPLAN_INTERVAL = 30, INFLATION_MARGIN = 0.0
  - Moving obstacles, SHIELD SDF, full disturbance, collision -250

Observation: 15 dims (3 obs x 3 + target 3 + velocity 2 + current alpha).
No phi in observation since it is fixed.
"""
...
class AlphaOnlyEnv(gym.Env):
    def __init__(self, systematic_bias_range=(-0.6, 0.4), jitter_range=(-0.2, 0.2),
        ...
        # Persistent alpha state (phi is fixed)
        ...
        # ACTION: Discrete(3) — 0=alpha_down, 1=alpha_stay, 2=alpha_up
        self.action_space = spaces.Discrete(3)

        # OBSERVATION: 3 obs x 3 + target 3 + velocity 2 + alpha 1 = 15
        self.observation_space = spaces.Box(
            low=np.array([-180, -35, 0, -180, -35, 0, -180, -35, 0,
                          -180, -35, 0.1, -7, -7,
                          ALPHA_MIN], dtype=np.float32),
            high=np.array([180, 35, 10, 180, 35, 10, 180, 35, 10,
                           180, 35, 5.0, 7, 7,
                           ALPHA_MAX], dtype=np.float32),
            dtype=np.float32,
        )

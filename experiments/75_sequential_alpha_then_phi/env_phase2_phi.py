"""
Exp 75, Phase 2: Learn phi with alpha frozen from Phase 1.

Loads the Phase 1 alpha-only model and uses it to set alpha at each step.
Only phi is learnable via discrete actions.

Key settings:
  - Alpha: frozen from Phase 1 model (queried each step)
  - Phi: learnable, Discrete(3) — 0=phi_down, 1=phi_stay, 2=phi_up
  - PHI_STEP = 0.5, PHI_INIT = 1.0
  - REPLAN_INTERVAL = 30, INFLATION_MARGIN = 0.0
  - Moving obstacles, SHIELD SDF, full disturbance, collision -250

Observation: 16 dims (3 obs x 3 + target 3 + velocity 2 + current alpha + current phi).
The phi policy sees everything including the current alpha value.
"""
...
# Alpha config (frozen from Phase 1)
ALPHA_STEP = 0.5
ALPHA_INIT = 2.0
...
class PhiWithFrozenAlphaEnv(gym.Env):
    def __init__(self, alpha_model_path="./models_dynamic/alpha_only_model",
        ...
        # Load frozen alpha model from Phase 1
        ...
        # ACTION: Discrete(3) — 0=phi_down, 1=phi_stay, 2=phi_up
    ...
    def _build_alpha_obs(self):
        """Build the 15-dim observation for the frozen alpha model."""
        parts = []
        for i in range(3):

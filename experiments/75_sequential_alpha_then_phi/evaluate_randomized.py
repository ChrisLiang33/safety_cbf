"""
Exp 75: Sequential Alpha->Phi -- fully randomized eval.

Loads the Phase 2 phi model, which internally uses the frozen Phase 1 alpha model.
A* replans every 30 steps with no inflation margin.
Alpha actions come from frozen Phase 1 model, phi actions from Phase 2 model.

3-column layout: trajectory | alpha+phi time series | alpha+phi actions (stacked)
...
from env_phase2_phi import PhiWithFrozenAlphaEnv, ALPHA_INIT, PHI_INIT

MODEL_PATH = "./models_dynamic/phi_frozen_alpha_model"
MAX_STEPS = 800
N_OBS = 3
...
def plot_discrete_actions(ax_alpha, ax_phi, scen, r, fontsize_title=12, fontsize_label=11):
    ...
    ax_alpha.set_title("Alpha Actions (frozen)", fontsize=fontsize_title)
    ...
    ax_phi.set_title("Phi Actions (learned)", fontsize=fontsize_title)
...
if __name__ == "__main__":
    ...
    print("Loading Phase 2 model (phi model with frozen alpha)...")
    env = PhiWithFrozenAlphaEnv()
    ...
    fig.suptitle("Exp 75: Sequential Alpha->Phi (degraded A*, step=0.5)",
                 fontsize=18, y=1.005)

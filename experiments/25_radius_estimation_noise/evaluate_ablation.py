"""
Phi-CBF + RADIUS ESTIMATION NOISE ablation:
  Dynamic [kx,ky,alpha,phi] (ISSf-CBF) vs Dynamic [kx,ky,alpha] (standard CBF)
Both trained and evaluated with obstacle radius estimation noise.

Key idea: CBF uses ESTIMATED radius (true + error), collision uses TRUE radius.
Eval uses worst-case error = -1.0 (underestimate: thinks obstacle is 4.0 when it's 5.0).
...
TRUE_RADIUS = 5.0
EVAL_RADIUS_ERROR = -1.0  # worst case: underestimate by 1.0 (thinks 4.0 when it's 5.0)
...
def run_phi_episode(env, model, scen, radius_error):
    ...
    for step in range(MAX_STEPS):
        ...
        # TRUE distances (for safety analysis)
        dists_true = [np.linalg.norm(env.robot_pos - env.obs_pos[i]) - env.true_radius[i]
                      for i in range(3)]
        dist_list_true.append(min(dists_true))
        # ESTIMATED distances (what CBF sees)
        dists_est = [np.linalg.norm(env.robot_pos - env.obs_pos[i]) - env.estimated_radius[i]
                     for i in range(3)]
        dist_list_est.append(min(dists_est))

        for oi in range(3):
            per_obs_dists_true[oi].append(dists_true[oi])
            per_obs_dists_est[oi].append(dists_est[oi])
            x_diff = env.robot_pos - env.obs_pos[oi]
            h = np.sum(x_diff**2) - env.estimated_radius[oi]**2
    ...
    return {
        ...
        "dist": dist_list_true, "dist_est": dist_list_est,
        "per_obs_dists": per_obs_dists_true, "per_obs_dists_est": per_obs_dists_est,
...
def run_alpha_only_episode(env, model, scen, radius_error):
    ...
    return {
        ...
        "safe_u_speeds": safe_u_speeds,
        "dist": dist_list_true, "dist_est": dist_list_est,
        "per_obs_dists": per_obs_dists_true,
        "total_reward": total_reward, "steps": step + 1,
        "reached_target": reached, "collided": collided,
        "min_clearance": min(dist_list_true), "path_length": plen,
        "path_efficiency": efficiency,
...
def generate_random_scenarios(n, seed=42):
    ...
    for i in range(n):
        ...
        # Per-scenario radius error (all obstacles get the same error for simplicity)
        radius_error = rng.uniform(-1.0, 0.0)  # always underestimate for eval
        scenarios.append({
            "name": f"Random_{i}",
...
if __name__ == "__main__":
    ...
    print(f"Loading models... (eval radius error={EVAL_RADIUS_ERROR}, estimated={EVAL_ESTIMATED_RADIUS})")
    phi_env = PhiCBFRadiusNoiseEnv()
    phi_model = PPO.load(PHI_MODEL_PATH)

    ao_env = AlphaOnlyRadiusNoiseEnv()
    ao_model = PPO.load(ALPHA_ONLY_MODEL_PATH)

    # --- Hand-picked scenarios ---
    # Worst case: underestimate radius by 1.0 (CBF thinks 4.0, true is 5.0)
    print(f"\nRunning {len(SCENARIOS)} hand-picked scenarios (radius error={EVAL_RADIUS_ERROR})...")
    all_scenarios = []
    for scen in SCENARIOS:
        phi_result = run_phi_episode(phi_env, phi_model, scen, EVAL_RADIUS_ERROR)
        ao_result = run_alpha_only_episode(ao_env, ao_model, scen, EVAL_RADIUS_ERROR)
    ...
    fig.suptitle(rf"RADIUS ESTIMATION NOISE (true={TRUE_RADIUS}, est={EVAL_ESTIMATED_RADIUS}, err={EVAL_RADIUS_ERROR}): "
                 rf"$\alpha$+$\varphi$ (ISSf-CBF) vs $\alpha$-only (standard CBF)",
                 fontsize=18, y=1.005)
    ...
    for i, data in enumerate(all_scenarios):
        ...
        ax = axs[i, 0]
        for j in range(3):
            # Show TRUE radius (solid red) and ESTIMATED radius (dashed gray)
        ...
        # Annotation showing the radius gap
        ax.text(5, -12, f"TRUE r={TRUE_RADIUS}, EST r={EVAL_ESTIMATED_RADIUS}\n"
                        f"Gap={TRUE_RADIUS - EVAL_ESTIMATED_RADIUS}m (danger zone)",
        ...
        for oi in range(3):
            d = phi["per_obs_dists"][oi]  # TRUE distances
        ...
        # --- Column 7: Policy Map (alpha & phi vs TRUE distance) ---
        ax = axs[i, 6]
        ...
        ax.axvline(TRUE_RADIUS - EVAL_ESTIMATED_RADIUS, color="orange", linewidth=1,
                   linestyle="--", alpha=0.5, label=f"EST thinks safe (+{TRUE_RADIUS - EVAL_ESTIMATED_RADIUS}m)")
    ...
    # =====================================================================
    # BATCH: 100 random scenarios (each with random radius underestimation)
    # =====================================================================
    ...
    for idx, scen in enumerate(random_scenarios):
        ...
        phi_r = run_phi_episode(phi_env, phi_model, scen, radius_error)
        m_phi = methods[1]
        ...
        ao_r = run_alpha_only_episode(ao_env, ao_model, scen, radius_error)
        m_ao = methods[0]
    ...
    fig.suptitle(rf"Aggregate -- RADIUS EST. NOISE (true={TRUE_RADIUS}, err$\sim$U(-1,0)): "
                 rf"$\alpha$+$\varphi$ vs $\alpha$-only ({N_RANDOM_SCENARIOS} Random Scenarios)",
                 fontsize=16)
    ...
    # =====================================================================
    print(f"\n{'='*100}")
    print(f"HAND-PICKED SCENARIO RESULTS (radius error={EVAL_RADIUS_ERROR}, est={EVAL_ESTIMATED_RADIUS})")
    print(f"{'='*100}")
    print(f"{'Scenario':<25} {'Method':<20} {'Reached':>8} {'Collided':>9} "
          f"{'TrueMinD':>9} {'Steps':>7} {'AvgSpd':>8} {'PathEff':>9}")
    print(f"{'-'*100}")
    ...
    print(f"{'='*100}")
    print(f"AGGREGATE ({N_RANDOM_SCENARIOS} RANDOM SCENARIOS, radius error ~ U(-1.0, 0.0))")
    print(f"{'='*100}")
    print(f"{'Method':<25} {'Success':>10} {'Collisions':>11} {'AvgTrueMinD':>12} "
    ...
    for m in methods:
        ...
        print(f"{m:<25} {a['success']:>8}/{N_RANDOM_SCENARIOS} "
              ...
              f"{np.mean(a['min_clearances']):>12.3f} {np.mean(a['steps']):>9.1f} "
